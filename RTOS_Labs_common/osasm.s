...
StartOS
; put your code here ------- pg 181 of textbook
    LDR     R0, =RunPt         ; currently running thread
    LDR     R1, [R0]           ; R1 = value of RunPt
    LDR     SP, [R1]           ; new thread SP; SP = RunPt->stackPtr;
...
; put your Lab 5 code here
	...
	LDM SP,{R0-R3} 				; Get any parameters	---these 4 lines are textbook code
	
	PUSH {LR}
	LDR LR, =Return				;when the functions return it jumps past the following switch case
	;switch case start
	CMP R12, #0					;these numbers are following the osam.s in the User directory
	BEQ OS_Id
	CMP R12, #1
	BEQ OS_Kill
	CMP R12, #2
...
Return	
	POP {LR}					;retrieve exception LR again
	STR R0,[SP]					;Store return value	---textbook line
