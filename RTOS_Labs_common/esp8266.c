    ...
    case 4:   // filter out data and put it into the right receive FIFO
      if(ReceiveDataLen) {
        switch(ReceiveDataStream) {
          case 0: {
						ESP8266Rx0Fifo_Put(letter);
						OS_Signal(&ESP8266Rx0);
						//if(ESP8266Rx0Fifo_Put(letter) == FIFOFAIL){  // overflow, data loss
...
void ESP8266_InitUART(int rx_echo, int tx_echo){ volatile int delay;
  ...
  ESP8266RxFifo_Init();
  ESP8266Rx0Fifo_Init();  
	OS_InitSemaphore(&ESP8266Rx, 0);
	OS_InitSemaphore(&ESP8266Rx0, 0);
	OS_InitSemaphore(&ESP8266Tx, FIFOSIZE-1);
  ESP8266_EchoResponse = rx_echo;
  ESP8266_EchoCommand = tx_echo;
#if ESP8266_UART==1
...
char ESP8266_InChar(void){ char letter;
	OS_Wait(&ESP8266Rx);
	ESP8266RxFifo_Get(&letter);
	//if(ESP8266RxFifo_Get(&letter) == FIFOSUCCESS)
...
int ESP8266_Receive(char* fetch, uint32_t max){ long sr; const char* s;
  ...
  while(max > 1) {
    if(ESP8266Rx0Fifo_Size() || ESP8266_DataAvailable) { // data (about to be) available?
      OS_Wait(&ESP8266Rx0);
			ESP8266Rx0Fifo_Get(&letter);
			//if(ESP8266Rx0Fifo_Get(&letter) == FIFOSUCCESS)	
...
int ESP8266_CloseTCPConnection(void){ int try=MAXTRY;
  while(try){
	...
	OS_InitSemaphore(&ESP8266Rx0, 0);
	ESP8266Rx0Fifo_Init(); //bug fix? piazza post @73
  return FAILURE;
}
