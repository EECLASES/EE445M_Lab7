...
#define KILLSTAT 0xFFFFFFFF
#define SLEEPPERIOD 80000000/1000 //1ms
#define STACKSIZE 512		//arbitrary number >= 16 for now
typedef struct TCB{
	uint32_t* stackPtr;
	struct TCB *next;
	struct TCB *prev;
	...
	uint32_t overflowcheckbot;
	uint32_t id;
	uint32_t sleep; //0xFFFF.FFFF means kill
	uint32_t priority;
	uint32_t blocked;
	struct PCB *pcb;
} TCB;
...
// ******** OS_Init_FileLock************	
// acquires lock and semaphore within	
// input:  pointer to a FileLock 	
// output: none	
void acquireFileLock(struct FileLock* fLock, struct File* file  ); // should wait here for lock using our OS_Wait	
...
// input:  pointer to a FileLock 	
// output: none	
void releaseFileLock(struct FileLock* fLock ); // should wait here for lock using our OS_Wait	
